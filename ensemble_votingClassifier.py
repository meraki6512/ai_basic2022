...
m2 = LogisticRegression(n_jobs=-1)
m3 = DecisionTreeClassifier(max_depth=3, random_state=1)
estimators = [('kn', m1), ('lr', m2), ('dt', m3)]
...
model.estimators_[0].predict(X_test[:1])
model.estimators_[1].predict(X_test[:1])
model.estimators_[2].predict(X_test[:1])
